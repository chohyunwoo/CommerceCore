...
- **2026-08-24 개정 — 임베딩 모델 CLIP → DINOv2(dinov2-small)**: 구현 후 CLIP(clip-vit-base-patch16)이 순수 image→image 시각 유사도에서 카테고리 분리가 약한 문제를 확인(같은/다른 카테고리 평균 유사도 0.70 vs 0.66). DINOv2는 텍스트 없이 자기지도로 학습돼 시각 리트리벌에 더 강함. **스파이크 실측**(상품 6장, 카테고리별 2장): 분리도(같음−다름) CLIP 0.037 → DINOv2 **0.145(약 4배)**, q8 다운로드 용량 84MB → **24MB**, 브라우저 실행 확인(transformers.js `image-feature-extraction`). 텍스트→이미지 검색은 미사용이라 CLIP 텍스트타워 손실 무관. 전환 내용: 프론트/오프라인 모두 dinov2-small(q8) + CLS 토큰(384차원)으로 통일(쿼리·카탈로그 정합성), `MIN_SIMILARITY` 0.5→0.15 재보정, 임베딩 차원 512→384(전 카탈로그 재계산). 오프라인 스크립트는 개별 이미지 실패 시 건너뛰도록 견고화. **프로덕션 후속(사용자)**: 배포 후 프로덕션 DB에도 `embeddings:compute` 재실행 필요(모델이 바뀌어 기존 512차원 임베딩과 섞이면 검색이 어긋남). (이슈 #96)
- **2026-08-24 임계값 재보정(이슈 #100)**: 전환 직후 `MIN_SIMILARITY=0.15`가 너무 낮아 무관 이미지(오리 캐릭터)가 운동화에 0.175로 오탐. **top-1 유사도 실측 스파이크**(무관 이미지 top1 ≤0.382 vs 실제 상품 사진 top1 ≥0.417, 카탈로그 원본 1.000)로 그 사이 간격을 확인해 **0.40**으로 상향 — 무관 쿼리는 결과 0개가 되고 실제 상품은 통과. 결과 0개 시 프론트 안내 문구도 "유사한 상품을 찾지 못했습니다"로 정정. (초기 0.15는 카테고리 평균 분포에서 추정한 값이라 실제 운영점과 달랐음)
- **2026-08-25 하이브리드 재개정(하드 컷오프 폐지)**: 사용자가 실제 스니커즈 사진을 올렸는데 사이트에 비슷한 신발이 있는데도 결과가 0개인 문제 제보. **프로덕션 카탈로그 임베딩(이미 DINOv2 384차원 50개)으로 유사도 분포 실측**(REST API로 조회 후 pairwise 코사인): 같은 카테고리끼리도 유사도 중앙값 0.18~0.30(신발↔신발 중앙값 **0.30**, 0.40 이상은 **35%뿐**), 다른 카테고리 중앙값 0.06이지만 일부 0.5+ 예외 존재 → **무관 점수대(~0.38)와 진짜 같은 카테고리 점수대(0.25~0.45)가 겹쳐 단일 임계값으로 정밀히 못 가름**(정밀도↔재현율 트레이드오프). 즉 0.40 하드 컷오프는 깨끗한 카탈로그 사진끼리도 같은 신발류를 대부분 걸러, 배경·각도가 다른 실제 업로드 사진은 전부 탈락시키고 있었음(0.417 기준은 카탈로그에 매우 가까운 쿼리에서만 나오는 값이었음). **선택(사용자 확정): 하이브리드** — 하드 컷오프 대신 명백히 무관한 것만 거르는 낮은 바닥값(`SIMILARITY_FLOOR=0.15`)만 두고 **항상 가장 비슷한 상위 5개를 반환**, 최고 점수가 `SIMILARITY_CONFIDENT=0.30` 미만이면 프론트가 "정확히 일치하는 상품은 없지만 비슷한 순" 안내를 표시(빈 결과는 바닥값도 못 넘는 진짜 무관 이미지에서만). 프로덕션 임베딩을 카탈로그 스니커즈로 시뮬레이션해 신발 쿼리에 신발 top-5(44~74%)가 반환되는 것 확인. **재검토 트리거**: 무관 업로드에 약한 매치가 자주 뜨는 게 문제되면 → 카테고리 사전 필터 또는 카테고리별 임계값 도입 검토.
- **2026-08-25 특징 추출 CLS → 패치 mean-pooling 전환**: "결과가 안 뜨는" 것과 별개로 "뜨는 결과의 품질"을 개선. 프로덕션 카탈로그 50개로 **모델·추출 방식 스파이크 비교**(node에서 transformers.js 실행, 신발 17개 leave-one-out + 카테고리 분리도 + 캔버스로우탑 스포트라이트): ①dinov2-small+CLS(현행) ②small+mean-pool ③dinov2-base+CLS ④base+mean-pool ⑤SigLIP ⑥CLIP-b32. **base/SigLIP/CLIP는 탈락** — q8에서 임베딩 공간이 붕괴하거나(base 분리도 0.03) 텍스트정렬 모델이라 신발 쿼리에 옷을 최상위로 올림(결정 32의 CLIP 배제 이유 재확인). **승자: small + 패치 mean-pooling** — 같은 모델·같은 24MB·같은 384차원(다운로드 변화 없음)에서 추출만 CLS→패치평균으로 바꿔, 캔버스로우탑 쿼리 top-5가 CLS의 "신발4(캔버스3)+부츠1"에서 "**신발5(캔버스4)**"로 개선되고 카테고리 분리도도 0.148→0.159로 소폭 상승. **카테고리 무관 균일 적용**(신발만이 아니라 전 카테고리 같은 추출) — leave-one-out 같은-카테고리 top-5 비율: 신발 92% / 상의 61% / 하의 59%(mean) ≈ CLS(93/61/56)로 옷·바지도 동급 이상. 전환: 프론트 `imageEmbedding.ts`·오프라인 `compute-product-embeddings.ts` 모두 mean-pooling으로 통일(쿼리·카탈로그 정합 필수), **프로덕션 카탈로그 임베딩 재계산 필요**(프론트 배포 직후 실행해야 어긋나지 않음). **알려진 한계(정직 기록)**: 이미지 검색 품질은 신발(92%)이 옷·바지(~60%)보다 확연히 좋음 — 옷 사진끼리 시각적으로 비슷해 DINOv2 어느 변형으로도 카테고리 혼동이 남으며, 이는 임베딩 방식이 아니라 모델·데이터(카탈로그 배경 제각각)의 한계. 더 끌어올리려면 카테고리 사전필터/재랭크 또는 배경 제거 전처리가 필요(별도 결정 사항).
